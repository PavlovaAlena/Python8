...
def write_csv(employees: list):
    with open(Path.cwd() / 'database.csv', 'w', encoding='utf-8') as fout:
        #csv_writer = csv.writer(fout) #Скорректировано, т.к. программа не пропускала наличие пустой строки в файле csv
...
def find_employee_by_last_name(employees: list, last_name: str) -> dict:
    employeeDic = []
    for employee in employees:
        #Скорректирован поиск, который остатнавливался на первой найденной записи.
        #return employee
        if employee['last_name'] == last_name:
            employeeDic.append(employee)        
    if len(employeeDic) == 0:
        return None
    else:
        return employeeDic
...
def del_employee(employees: list, employee: dict):
    #employees.remove(employee) #Добавлена проверка на нахождение удаляемой записи, что бы программа не вылетала по ошибки, если таковая не найдена
    if employee not in employees:
        print("Записи с перечисленными данными нет в базе")
    else:    
